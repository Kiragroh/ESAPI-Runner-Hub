...
- Removed internal deployment plans, organization-specific paths, hosts,
  assembly metadata, and catalogue labels from the public repository.
- Replaced live defaults with explicit configuration or synthetic examples.
- Added an independence and no-warranty disclaimer without changing the core
  launcher or ESAPI execution model.
...
- Direct patient-context catalogue entries cover example single-file and compiled DICOM exporters.
...
- Compile-time authorization metadata and deployment configuration use the Eclipse 18 API/Types pair selected by the operator.
