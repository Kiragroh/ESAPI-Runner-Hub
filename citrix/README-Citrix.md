...
```text
\\fileserver\clinical-tools\ESAPI-Runner-Hub\citrix\ESAPI-Runner-Hub.CitrixLauncher.exe
```
...
```text
\\fileserver\clinical-tools\ESAPI-Runner-Hub\citrix
```
...
```text
/d /c call "\\fileserver\clinical-tools\ESAPI-Runner-Hub\citrix\Start-ESAPI-Runner-Hub.cmd"
```
...
```powershell
& '..\tools\Invoke-CitrixContextDebug.ps1' -ApplicationId plugin-color-code -PatientId PATIENT-ID -CourseId C7 -PlanId PLAN-ID -RequestDirectory '\\fileserver\clinical-tools\ESAPI-Runner-Hub\requests'
```
...
```ini
LogDirectory=\\fileserver\clinical-tools\ESAPI-Runner-Hub\logs
ContextRequestDirectory=\\fileserver\clinical-tools\ESAPI-Runner-Hub\requests
```

A direct VDA shell does not depend on the Studio placeholder, for example:

```powershell
cmd.exe /d /s /c ""\\fileserver\clinical-tools\ESAPI-Runner-Hub\citrix\Start-ESAPI-Runner-Hub.cmd" --replay-latest plugin-fb-info"
```
...
cmd.exe /d /s /c ""\\fileserver\clinical-tools\ESAPI-Runner-Hub\citrix\Start-ESAPI-Runner-Hub.cmd" --run-context plugin-fb-info"
...
cmd.exe /d /s /c ""\\fileserver\clinical-tools\ESAPI-Runner-Hub\citrix\Start-ESAPI-Runner-Hub.cmd" --run-contexts plugin-fb-info"
...
```powershell
$citrix = '\\fileserver\clinical-tools\ESAPI-Runner-Hub\citrix'
...
```powershell
cmd.exe /d /s /c ""\\fileserver\clinical-tools\ESAPI-Runner-Hub\citrix\Start-ESAPI-Runner-Hub.cmd" --offline-ui-smoke"
```
